...
describe TotalCalculator do
  ...
  subject { TotalCalculator.new(test_products, promotions,
                                price_calculator_klass) }

  before do
    allow(price_calculator_klass).to receive(:new)
      .with(test_products, promotions).and_return(price_calculator)
    allow(price_calculator).to receive(:get_price).with(:'001', 2)
      .and_return(price)
    allow(promotions).to receive(:get_discount_rate).with(basket_value)
      .and_return(0.1)
  end
